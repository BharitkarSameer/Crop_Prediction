...
with col_lat:
    lat_input = st.number_input("Latitude", value=float(detected_lat), format="%.4f", step=0.0001)
with col_lon:
    lon_input = st.number_input("Longitude", value=float(detected_lon), format="%.4f", step=0.0001)
...
col1, col2, col3 = st.columns(3)
with col1:
    N = st.number_input("Nitrogen (N) kg/ha", 0.0, 200.0, float(st.session_state['N']), 1.0)
with col2:
    P = st.number_input("Phosphorus (P) kg/ha", 0.0, 200.0, float(st.session_state['P']), 1.0)
with col3:
    K = st.number_input("Potassium (K) kg/ha", 0.0, 200.0, float(st.session_state['K']), 1.0)
st.markdown('</div>', unsafe_allow_html=True)
